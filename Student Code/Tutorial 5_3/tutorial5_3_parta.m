...
for i = 2:length(t)            % loop through all time points
    ...
    V1(i) = V1(i-1) + dt*((E_L - V1(i-1))/R + I1app(i-1) +G21*syn1(i-1)*(E21_rev -V1(i-1)) + noise)/Cm;
    ...
    if V1(i) > Vth              % if potential is above threshold
        ...
        syn1(i) = syn1(i-1) + p_r*D1(i-1)*(1-syn1(i-1));
        D1(i) = D1(i-1)*(1-p_r);
    end
    ...
    %repeat for cell 2
     V2(i) = V2(i-1) + dt*((E_L - V2(i-1))/R + I2app(i-1) +G12*syn2(i-1)*(E12_rev -V2(i-1)) + noise)/Cm;
    ...
    if V2(i) > Vth              % if potential is above threshold
        ...
        syn2(i) = syn2(i-1) + p_r*D2(i-1)*(1-syn2(i-1));
        D2(i) = D2(i-1)*(1-p_r);
    end
