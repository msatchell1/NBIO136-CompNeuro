...
% 
% Unit 1 is excitatory, coupled to Unit 2, which is inhibitory such that
% the system produces gamma oscillations via the PING mechanism.
%
% This code is a solution to Tutorial 6.3 of the textbook
...
% Based on question_part, set the
% Mean excitatory input conductance to E-Unit (1), G_E_input_vec and
% Mean excitatory input conductance to I-Unit (2), G_I_input_vec
switch question_part
    case {1,2,3}
        G_E_input_vec = 1e-9;
        G_I_input_vec = 0;
    case 4
        G_E_input_vec = 1e-9*[0:0.2:10.0];
        G_I_input_vec = zeros(size(G_E_input_vec));
    case 5
        G_I_input_vec = 1e-12*[0:1:20];
        G_E_input_vec = 1e-9*ones(size(G_I_input_vec));
    otherwise
        disp('Invalid question part')
end
...
for parameter = 1:Nparameters           % Loop through stimulus values
    ...
    f_alt(parameter) = f_vec(f_index+1);
...
if ( question_part < 4 )        % Plot the rates vs time
    figure(1)
    plot(tvec, r(1,:))
    hold on
    plot(tvec,r(2,:))
...
if ( question_part == 3 )       % Plot result of Fourier transform
    figure(2)
    plot(f_vec,power)
    xlabel('Frequency, Hz')
    ylabel('Power')
    title('Power spectrum for oscillations')
end

if ( question_part == 4 )       % Plot oscillation properties vs input
    figure(98)
    clf
    subplot('Position',[0.255 0.72 0.7 0.25])
    plot(G_E_input_vec*1e9, mean_f,'k')
    hold on
    plot(G_E_input_vec*1e9, f_alt,'k--')
    axis([0 max(G_E_input_vec*1e9) 0 50])
    ylabel({'Oscillation'; 'frequency'; '(Hz)'})
    
    subplot('Position',[0.25 0.41 0.7 0.25])
    plot(G_E_input_vec*1e9,r_amp1,'k')
    hold on
    plot(G_E_input_vec*1e9,r_amp2,'k:')
    ylabel({'Oscillation'; 'amplitude'})
    legend('E Unit', 'I Unit')
    axis([0 max(G_E_input_vec*1e9) 0 200])
    
    subplot('Position',[0.25 0.1 0.7 0.25])
    plot(G_E_input_vec*1e9,mean_r1,'k')
    hold on
    plot(G_E_input_vec*1e9,mean_r2,'k:')
    ylabel({'Mean'; 'firing rate'; '(Hz)'})
    xlabel('E Input Conductance (nS)')
    legend('E Unit', 'I Unit')
    axis([0 max(G_E_input_vec*1e9) 0 80])
end

if ( question_part == 5 )       % Plot oscillation properties vs input
    figure(99)
    clf
    subplot('Position',[0.255 0.72 0.7 0.25])
    plot(G_I_input_vec*1e12, mean_f,'k')
    hold on
    plot(G_I_input_vec*1e12, f_alt,'k--')
    axis([0 max(G_I_input_vec*1e12) 0 50])
    ylabel({'Oscillation'; 'frequency'; '(Hz)'})
    
    subplot('Position',[0.25 0.41 0.7 0.25])
    plot(G_I_input_vec*1e12,r_amp1,'k')
    hold on
    plot(G_I_input_vec*1e12,r_amp2,'k:')
    ylabel({'Oscillation'; 'amplitude'})
    legend('E Unit', 'I Unit')
    axis([0 max(G_I_input_vec*1e12) 0 200])
    
    subplot('Position',[0.25 0.1 0.7 0.25])
    plot(G_I_input_vec*1e12,mean_r1,'k')
    hold on
    plot(G_I_input_vec*1e12,mean_r2,'k:')
    ylabel({'Mean'; 'firing rate'; '(Hz)'})
    xlabel('I Input Conductance (pS)')
    legend('E Unit', 'I Unit')
    axis([0 max(G_I_input_vec*1e12) 0 80])
end
